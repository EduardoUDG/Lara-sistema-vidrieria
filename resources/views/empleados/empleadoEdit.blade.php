            ...
            <form action="{{ route('empleados.update', $empleado) }}" method="POST">
              ...
              <div class="mb-3 input-group-md">
                ...
                <select class="form-select" name="role_id">
                    <option value="{{ $empleado->roles->id }}">{{ $empleado->roles->nombre }}</option>
                    @foreach ($roles as $role)
                        <option value="{{ old('role_id', $role->id) }}">{{ $role->nombre }}</option>
                    @endforeach
                </select>
                @error('role_id')
                    <small class="text-danger tet-sm">
              ...
              <div class="mb-3 input-group-md">
                ...
                <select class="form-select" name="seguro_id">
                    <option value="{{ old('seguro_id', $empleado->seguros->id) }}">{{ old('seguro_id', $empleado->seguros->numero) }}</option>
                    @foreach ($seguros as $seguro)
                        <option value="{{ old('seguro_id', $seguro->id) }}">{{ $seguro->numero }}</option>
                    @endforeach
                </select>
                @error('seguro_id')
                    <small class="text-danger tet-sm">
